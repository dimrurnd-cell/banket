...
**2. Имена полей.** Если назвать поля так же, как ниже, подбор зала будет
подставлять в форму число гостей, а кнопки на страницах форматов —
вид мероприятия:

| Имя поля в Tilda | Что подставляется |
|------------------|-------------------|
| `Выберите количество гостей` | число со шкалы в блоке «Подбор зала» |
| `мероприятие` | формат со страницы (Свадьба, Корпоратив, Поминальный обед…) |

Остальные поля можно называть как удобно. Для совместимости со старыми
заявками прежняя форма сайта использовала: `Name`, `messenger-type`,
`messenger-id`, `Выберите дату мероприятия`, `Email`, `Комментарий`,
`soglasiye-persdan`.
...
подставляются в поля штатной формы.
