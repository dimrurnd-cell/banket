...
Чтобы добавить или убрать кадры, залейте файлы со следующими номерами и
поправьте `GALLERY` в `_src/data.py` (там же задаётся диапазон номеров).
Порядок на странице — по номеру файла.

Фильтры («Залы», «Свадьбы», «Корпоративы»…) выключены: по именам файлов
нельзя понять, что на кадре. Чтобы включить их, распишите номера по форматам
в `GALLERY_GROUPS` в `_src/data.py`, например `{"svadba": [1, 4, 9]}` —
